...
class Dictionary {
  ...
  _getOrCacheAllTerms(dictName) {
    ...
    if (!allTerms) {
      ...
      log('INFO', `辞書「${dictName}」から${allTerms.length}個の用語を読み込みました`);
      if (allTerms.length > 0) {
        log('INFO', `辞書サンプル: ${allTerms.slice(0, 3).map(t => `"${t.source}" -> "${t.target}"`).join(', ')}`);
      }
  ...
  addTerms(dictName, terms) {
    ...
    try {
      ...
      log('INFO', `既存用語数: ${existingTerms.size}`);
      
      // 完全一致のみで重複チェック
      const newTerms = terms.filter(term => {
        const key = `${term.source}_${term.target}`;
        const isDuplicate = existingTerms.has(key);
        if (isDuplicate) {
          log('INFO', `重複により除外: "${term.source}" -> "${term.target}"`);
        }
        return !isDuplicate;
      });
      
      log('INFO', `新規用語: ${terms.length}個中${newTerms.length}個を追加対象とします`);
      
      if (newTerms.length === 0) {
        return { 
          success: true, 
          added: 0,
          message: '新規用語がありませんでした'
        };
      }
      ...
      return {
        ...
        message: `${newTerms.length}個の新規用語を追加しました`
      };
